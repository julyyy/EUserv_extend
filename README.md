...
4、仓库内包含的 GitHub Actions 配置文件会在每天国际标准时间 0点50分。自动执行检查账号的脚本文件，如果检查到有需要续期的VPS，会自动续期，你也可以通过 `Push` 操作手动触发执行（建议修改脚本执行时间，修改action.yml，分析一下国际标准时间和德国机房的时差，选个夜深人静的时候自动执行吧，本脚本内已改为每天国际标准时间 1点50分，也就是德国2点50分）。
